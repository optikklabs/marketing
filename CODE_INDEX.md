...
| **Marketing** | `marketing/` | `/`, `/features`, `/pricing`, `/cli`, `/opentelemetry`, `/self-host`, `/architecture`, `/privacy`, `/terms`, `/dpa`, `/support`, `/security` | Public-facing site marketing pages, positioned as AI-native, terminal-first observability. Datadog-style motion restraint: the only animation is a one-time `motion/Reveal` fade (10px, 0.5s); `GradientText` is a static gradient; no marquee/count-ups/tilt/stagger. CLI sessions render in `visuals/TerminalWindow.tsx`; mock incident data is unified on one story (MySQL lock-wait in payment-svc after deploy `abc12d`, trace `9f3c21ab`). Homepage is 7 sections + CTA (Hero, LogoStrip static row, AgentTerminal, platform bento, 3 splits, OpenSource, CTA); hero headline is "Observability for the AI era". The hero and the three feature splits (logs/traces/metrics) all render `visuals/CliUiVisual.tsx` - a stacked `.m-cli-ui` pane showing the CLI session over the equivalent web-UI `Screenshot` (hero uses the `services` shot), so both CLI and UI show together. GitHub STAR count (GitHub API wrapper `useGitHubStars`) renders only in `OpenSourceSection` - the nav star badge was removed. `/pricing` (PricingPage) presents Launch, Scale, and Enterprise managed plans plus annual usage tiers beginning at $0.25/GB for logs, traces, and profiles and $1.80/1K ATS/hour for metrics; self-hosting remains available on its dedicated page. `/support` (SupportPage) publishes the `support@optikk.in` inbox + S1–S4 severity/response matrix; `/dpa` (DPAPage, in the `Legal` layout alongside privacy/terms/security) is the draft Data Processing Addendum (flagged pending legal review); `/cli` (CliPage) documents the `optikk` CLI with realistic command+output examples (data in `CliPageData.ts` - keep in sync with the CLI repo's `cmd/`). Query/Fleet/Operate sections pair the terminal `CodeBlock` with the matching web-UI `Screenshot` in an `m-split` grid (terminal + UI side by side). A `CommandReference` section (`CommandReference.tsx` + `CommandReferenceData.ts`, `.m-cmdref` card grid, anchor `#commands`) lists every command group and subcommand mirrored from `optikk --help` - keep in sync with the CLI repo's `cmd/`. Install is curl-only (`curl -fsSL https://optikk.in/install.sh | sh` - firebase.json redirects `/install.sh` to the raw `optikk/install.sh` on GitHub). |
